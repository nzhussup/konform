...
func TestMinLen(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "missing min_len rule is ignored",
			field:     makeStringField("Name", "abc", nil),
			wantTotal: 0,
		},
		{
			name:      "length equal to min_len passes",
			field:     makeStringField("Name", "abc", map[string]string{"min_len": "3"}),
			wantTotal: 0,
		},
		{
			name:      "length greater than min_len passes",
			field:     makeStringField("Name", "abcd", map[string]string{"min_len": "3"}),
			wantTotal: 0,
		},
		{
			name:        "length smaller than min_len returns validation min len",
			field:       makeStringField("Name", "ab", map[string]string{"min_len": "3"}),
			wantTotal:   1,
			wantErrType: errs.ValidationMinLen,
			wantLike:    "expected minimum length 3 but got 2",
		},
		{
			name:        "invalid min_len arg returns invalid schema",
			field:       makeStringField("Name", "abc", map[string]string{"min_len": "abc"}),
			wantTotal:   1,
			wantErrType: errs.InvalidSchema,
			wantLike:    `invalid min_len value "abc"`,
		},
		{
